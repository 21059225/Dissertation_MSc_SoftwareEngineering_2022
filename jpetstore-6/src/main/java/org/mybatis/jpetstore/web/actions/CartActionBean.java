/**
 *    Copyright (C) 2010-2017 the original author or authors.
 *                  2018 iObserve Project (https://www.iobserve-devops.net)
 *
 *    Licensed under the Apache License, Version 2.0 (the "License");
 *    you may not use this file except in compliance with the License.
 *    You may obtain a copy of the License at
 *
 *       http://www.apache.org/licenses/LICENSE-2.0
 *
 *    Unless required by applicable law or agreed to in writing, software
 *    distributed under the License is distributed on an "AS IS" BASIS,
 *    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *    See the License for the specific language governing permissions and
 *    limitations under the License.
 */
package org.mybatis.jpetstore.web.actions;

import java.util.Iterator;
import java.util.UUID;

import javax.servlet.http.HttpServletRequest;

import org.mybatis.jpetstore.domain.Cart;
import org.mybatis.jpetstore.domain.CartItem;
import org.mybatis.jpetstore.domain.Item;
import org.mybatis.jpetstore.service.CatalogService;

import kieker.common.record.controlflow.OperationExecutionRecord;
import kieker.monitoring.core.controller.IMonitoringController;
import kieker.monitoring.core.controller.MonitoringController;
import net.sourceforge.stripes.action.ForwardResolution;
import net.sourceforge.stripes.action.Resolution;
import net.sourceforge.stripes.action.SessionScope;
import net.sourceforge.stripes.integration.spring.SpringBean;

/**
 * The Class CartActionBean.
 *
 * @author Eduardo Macarron
 */
@SessionScope
public class CartActionBean extends AbstractActionBean {
	
	private static final IMonitoringController MONITORING_CONTROLLER = MonitoringController.getInstance();
    private static final long serialVersionUID = -4038684592582714235L;

    private static final String VIEW_CART = "/WEB-INF/jsp/cart/Cart.jsp";
    private static final String CHECK_OUT = "/WEB-INF/jsp/cart/Checkout.jsp";

    @SpringBean
    private transient CatalogService catalogService;

    private Cart cart = new Cart();
    private String workingItemId;

    public Cart getCart(){
    	
        return cart;
    }

    public void setCart(final Cart cart){
    	
        this.cart = cart;
    }

    public void setWorkingItemId(final String workingItemId){
    	
        this.workingItemId = workingItemId;
    }
    
    
	

    /**
     * Adds the item to cart.
     *
     * @return the resolution
     */
    public Resolution addItemToCart(){
    	UUID uuid = UUID.randomUUID();
    	cart.setSessionId(uuid.toString());
        if (cart.containsItemId(workingItemId)){
            cart.incrementQuantityByItemId(workingItemId,cart.getSessionId());
        } else {
            // isInStock is a "real-time" property that must be updated
            // every time an item is added to the cart, even if other
            // item details are cached.
            final boolean isInStock = catalogService.isItemInStock(workingItemId,cart.getSessionId());
            final Item item = catalogService.getItem(workingItemId,cart.getSessionId());
            cart.addItem(item, isInStock,cart.getSessionId());
        }
    
        final long tin = MONITORING_CONTROLLER.getTimeSource().getTime();
        final long tout = MONITORING_CONTROLLER.getTimeSource().getTime();
        final OperationExecutionRecord e = new OperationExecutionRecord(
                "public Resolution " + this.getClass().getName() +
                " addItemToCart()",
                uuid.toString(),
                OperationExecutionRecord.TRACE_ID,
                tin, tout, "myHost",
                1,
                1);
        MONITORING_CONTROLLER.newMonitoringRecord(e);
        
        return new ForwardResolution(CartActionBean.VIEW_CART);
    }

    /**
     * Removes the item from cart.
     *
     * @return the resolution
     */
    public Resolution removeItemFromCart(){
    	UUID uuid = UUID.randomUUID();
        final Item item = cart.removeItemById(workingItemId);
        item.setSessionId( uuid.toString());
        final long tin = MONITORING_CONTROLLER.getTimeSource().getTime();
        final long tout = MONITORING_CONTROLLER.getTimeSource().getTime();
        final OperationExecutionRecord e = new OperationExecutionRecord(
                "public Resolution " + this.getClass().getName() +
                " removeItemFromCart()",
                uuid.toString(),
                OperationExecutionRecord.TRACE_ID,
                tin, tout, "myHost",
                1,
                1);
        MONITORING_CONTROLLER.newMonitoringRecord(e);

        if (item == null){
            setMessage("Attempted to remove null CartItem from Cart.");
            return new ForwardResolution(AbstractActionBean.ERROR);
        } else {
            return new ForwardResolution(CartActionBean.VIEW_CART);
        }
    }

    /**
     * Update cart quantities.
     *
     * @return the resolution
     */
    public Resolution updateCartQuantities(){
    	UUID uuid = UUID.randomUUID();
    	final long tin = MONITORING_CONTROLLER.getTimeSource().getTime();
        final long tout = MONITORING_CONTROLLER.getTimeSource().getTime();
        final OperationExecutionRecord e = new OperationExecutionRecord(
                "public Resolution " + this.getClass().getName() +
                " updateCartQuantities()",
                uuid.toString(),
                OperationExecutionRecord.TRACE_ID,
                tin, tout, "myHost",
                1,
                1);
        MONITORING_CONTROLLER.newMonitoringRecord(e);
        
        final HttpServletRequest request = context.getRequest();

        final Iterator<CartItem> cartItems = getCart().getAllCartItems();
        while (cartItems.hasNext()){
            final CartItem cartItem = cartItems.next();
            final String itemId = cartItem.getItem().getItemId();
            try {
                final int quantity = Integer.parseInt(request.getParameter(itemId));
                getCart().setQuantityByItemId(itemId, quantity, uuid.toString());
                if (quantity < 1){
                    cartItems.remove();
                }
            } catch (final Exception e1){
                // ignore parse exceptions on purpose
            }
        }
           
        return new ForwardResolution(CartActionBean.VIEW_CART);
    }

    public ForwardResolution viewCart(){
    	UUID uuid = UUID.randomUUID();
    	final long tin = MONITORING_CONTROLLER.getTimeSource().getTime();
        final long tout = MONITORING_CONTROLLER.getTimeSource().getTime();
        final OperationExecutionRecord e = new OperationExecutionRecord(
                "public ForwardResolution " + this.getClass().getName() +
                " viewCart()",
                uuid.toString(),
                OperationExecutionRecord.TRACE_ID,
                tin, tout, "myHost",
                1,
                1);
        MONITORING_CONTROLLER.newMonitoringRecord(e);
        return new ForwardResolution(CartActionBean.VIEW_CART);
    }

    public ForwardResolution checkOut(){
    	UUID uuid = UUID.randomUUID();
    	final long tin = MONITORING_CONTROLLER.getTimeSource().getTime();
        final long tout = MONITORING_CONTROLLER.getTimeSource().getTime();
        final OperationExecutionRecord e = new OperationExecutionRecord(
                "public ForwardResolution " + this.getClass().getName() +
                " checkOut()",
                uuid.toString(),
                OperationExecutionRecord.TRACE_ID,
                tin, tout, "myHost",
                1,
               1);
        MONITORING_CONTROLLER.newMonitoringRecord(e);
        return new ForwardResolution(CartActionBean.CHECK_OUT);
    }

    public void clear(){
    	
        cart = new Cart();
        workingItemId = null;
    }

}
